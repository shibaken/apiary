        ...
        <div class="col-md-3">
          <div class="mb-3">
            ...
            <select class="form-select" v-model="filterProposalStatus">
              ...
              <option v-for="s in approval_status" :value="s" :key="s">
                {{ s }}
              </option>
            </select>
          </div>
  ...
  watch: {
    filterProposalRegion: function () {
      ...
      if (vm.filterProposalRegion != "All") {
        vm.$refs.proposal_datatable.vmDataTable
          .column("current_proposal__region__name:name")
          .search(vm.filterProposalRegion)
          .draw();
      } else {
        vm.$refs.proposal_datatable.vmDataTable
          .column("current_proposal__region__name:name")
    ...
    filterProposalActivity: function () {
      ...
      if (vm.filterProposalActivity != "All") {
        vm.$refs.proposal_datatable.vmDataTable
          .column("current_proposal__activity:name")
    ...
    filterProposalExpiryFrom: {
      get() {
        ...
        if (this.proposal_expiry_from) {
          return moment(this.proposal_expiry_from, "YYYY-MM-DD").format(
            "DD/MM/YYYY",
          );
        }
    ...
    filterProposalExpiryTo: {
      get() {
        ...
        if (this.proposal_expiry_to) {
          return moment(this.proposal_expiry_to, "YYYY-MM-DD").format(
            "DD/MM/YYYY",
          );
        }
        return ""; // Otherwise, return an empty string.
    ...
    filterProposalStartFrom: {
      get() {
        ...
        if (this.proposal_start_from) {
          return moment(this.proposal_start_from, "YYYY-MM-DD").format(
            "DD/MM/YYYY",
          );
        }
        return ""; // Otherwise, return an empty string.
    ...
    filterProposaStartTo: {
      get() {
        ...
        if (this.proposal_start_to) {
          return moment(this.proposal_start_to, "YYYY-MM-DD").format(
            "DD/MM/YYYY",
          );
        }
        return ""; // Otherwise, return an empty string.
              ...
              if (full.can_reissue) {
                if (!full.can_action) {
                  if (full.set_to_cancel) {
                    message =
                      "This Approval is marked for cancellation to future date";
                  }
                  if (full.set_to_suspend) {
                    message =
                      "This Approval is marked for suspension to future date";
                  }
                  if (full.set_to_surrender) {
                    message =
                      "This Approval is marked for surrendering to future date";
                  }
  ...
  methods: {
      ...
      this.proposal_options = {
        ...
        ajax: {
          ...
          data: function (d) {
            ...
            d.start_date_from =
              vm.proposal_start_from != "" && vm.proposal_start_from != null
                ? moment(vm.proposal_start_from, "YYYY-MM-DD").format(
                    "YYYY-MM-DD",
                  )
                : "";
            d.start_date_to =
              vm.proposal_start_to != "" && vm.proposal_start_to != null
                ? moment(vm.proposal_start_to, "YYYY-MM-DD").format(
                    "YYYY-MM-DD",
                  )
                : "";
            ...
            d.expiry_date_from =
              vm.proposal_expiry_from != "" && vm.proposal_expiry_from != null
                ? moment(vm.proposal_expiry_from, "YYYY-MM-DD").format(
                    "YYYY-MM-DD",
                  )
                : "";
            d.expiry_date_to =
              vm.proposal_expiry_to != "" && vm.proposal_expiry_to != null
                ? moment(vm.proposal_expiry_to, "YYYY-MM-DD").format(
                    "YYYY-MM-DD",
                  )
                : "";
    ...
    setDashboardText: function () {
      ...
      this.dashboardDescription =
        "View existing licences and amend or renew them";
    },

    fetchFilterLists: function () {
      let vm = this;

      fetch(api_endpoints.filter_list_approvals)
        .then(async (response) => {
          if (!response.ok) {
