...
if (mountTarget) {
  ...
  window.fetch = ((orig) => {
    return async (...args) => {
      // 1. Safely normalize URL (supports Request objects, URLs, and strings)
      let url;
      ...
      // 3. Only attach CSRF token to same-origin requests
      if (sameOrigin) {
        headers.set("X-CSRFToken", helpers.getCookie("csrftoken"));
      }

      // 4. Handle JSON Content-Type
      if (
        sameOrigin &&
        args.length > 1 &&
      ...
      // 5. Allow browser to set boundary for FormData
      const requestBody =
      ...
      // 6. Set headers back to arguments
      if (args.length > 1) {
      ...
      // 8. Only redirect on 401/403 if it is a same-origin API endpoint
      if (
